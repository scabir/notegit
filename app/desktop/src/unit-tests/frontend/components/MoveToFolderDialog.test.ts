...
describe("MoveToFolderDialog", () => {
  it("calls onConfirm with root when confirmed", () => {
    ...
    const confirmButton = buttons.find(
      (button) => button.props.children === MOVE_DIALOG_TEXT.confirm,
    );
  ...
  it("shows error when destination is same as current location", () => {
    ...
    const treeView = renderer.root.findByType(
      require("@mui/x-tree-view").TreeView,
    );
    act(() => {
  ...
  it("moves to a selected folder and supports cancel", () => {
    ...
    const cancelButton = buttons.find(
      (button) => button.props.children === MOVE_DIALOG_TEXT.cancel,
    );
    const confirmButton = buttons.find(
      (button) => button.props.children === MOVE_DIALOG_TEXT.confirm,
    );
