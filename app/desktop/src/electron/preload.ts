...
const api: NoteBranchApi = {
  ...
  config: {
    ...
    updateAppSettings: (settings) =>
      ipcRenderer.invoke("config:updateAppSettings", settings),
    updateRepoSettings: (settings) =>
      ipcRenderer.invoke("config:updateRepoSettings", settings),
    getFavorites: () => ipcRenderer.invoke("config:getFavorites"),
    updateFavorites: (favorites) =>
      ipcRenderer.invoke("config:updateFavorites", favorites),
    ...
    createProfile: (name, repoSettings) =>
      ipcRenderer.invoke("config:createProfile", name, repoSettings),
    deleteProfile: (profileId) =>
      ipcRenderer.invoke("config:deleteProfile", profileId),
    setActiveProfile: (profileId) =>
      ipcRenderer.invoke("config:setActiveProfile", profileId),
  ...
  files: {
    ...
    saveAs: (repoPath, destPath) =>
      ipcRenderer.invoke("files:saveAs", repoPath, destPath),
    duplicate: (repoPath) => ipcRenderer.invoke("files:duplicate", repoPath),
    import: (sourcePath, targetPath) =>
      ipcRenderer.invoke("files:import", sourcePath, targetPath),
  },
  dialog: {
  ...
  history: {
    ...
    getVersion: (commitHash, path) =>
      ipcRenderer.invoke("history:getVersion", commitHash, path),
    getDiff: (hash1, hash2, path) =>
      ipcRenderer.invoke("history:getDiff", hash1, hash2, path),
  },
  search: {
    query: (query, options) =>
      ipcRenderer.invoke("search:query", query, options),
    repoWide: (query, options) =>
      ipcRenderer.invoke("search:repoWide", query, options),
...
contextBridge.exposeInMainWorld("NoteBranchApi", api);
