...
test("(git) restart app and recover git workspace", async ({
  request: _request,
}, testInfo) => {
...
test("(git) parallel app instances stay isolated by userData path", async ({
  request: _request,
}) => {
...
test("(git) restart preserves uncommitted changes and git status", async ({
  request: _request,
}, testInfo) => {
  const userDataDir = await createIsolatedUserDataDir(testInfo);
  let firstApp: ElectronApplication | null = null;
...
test("(git) restart loads repository for active profile", async ({
  request: _request,
}, testInfo) => {
  const userDataDir = await createIsolatedUserDataDir(testInfo);
  let firstApp: ElectronApplication | null = null;
