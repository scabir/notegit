...
export const resolveMarkdownImageSrc = (
  ...
  const relativeSrc = normalizedSrc.startsWith("/")
...
export const resolveMarkdownLinkedFilePath = (
  ...
  const normalizedHrefPath = normalizeFileSystemPath(
    decodePathSafely(hrefWithoutQuery),
  );
  ...
  const baseSegments = isRepoRootPath
    ? []
    : splitPathSegments(getFileDirectoryPath(markdownFilePath));
