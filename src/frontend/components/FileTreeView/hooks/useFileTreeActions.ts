...
import { mapCreateItemError, mapRenameError, getOperationErrorMessage } from '../errorHelpers';
import { resolveImportTargetPath, resolveParentDestination } from '../pathResolvers';
...
export function useFileTreeActions({
  ...
  const handleRename = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage(mapRenameError(error, selectedNode.type, FILE_TREE_MESSAGES.failedRename));
    } finally {
      setCreating(false);
    }
  }, [
  ...
  const handleDelete = useCallback(
    ...
    [onDelete, removeFavoritesUnderPath, selectedNodeForActions, setSelectedNode]
  );
