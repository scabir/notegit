...
describe('repoHandlers', () => {
  ...
  it('returns ok for openOrClone', async () => {
    ...
    const response = await handlers['repo:openOrClone'](null, { provider: 'git' });
  ...
  it('returns status on repo:getStatus', async () => {
    const { ipcMain, handlers } = createIpcMain();
    const repoService = {
      getStatus: jest.fn().mockResolvedValue({ provider: 'git', ahead: 0, behind: 0 }),
    } as any;
    ...
    expect(response.ok).toBe(true);
    expect(response.data?.provider).toBe('git');
  });
  ...
  it('returns ok for repo:fetch', async () => {
    const { ipcMain, handlers } = createIpcMain();
    const repoService = {
      fetch: jest.fn().mockResolvedValue({ provider: 'git' }),
    } as any;
