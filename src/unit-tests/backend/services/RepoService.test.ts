...
describe('RepoService', () => {
  ...
  it('throws for unsupported providers', async () => {
    ...
    const mockConfigService = {
      updateRepoSettings: jest.fn(),
      getAppSettings: jest.fn().mockResolvedValue({ s3AutoSyncEnabled: false, s3AutoSyncIntervalSec: 30 }),
    } as unknown as ConfigService;
    const mockFsAdapter = {
      mkdir: jest.fn(),
      exists: jest.fn().mockResolvedValue(false),
  ...
  it('generates a local path for s3 repos when missing', async () => {
    const { repoService, s3Provider, mockConfigService, mockFsAdapter } = createRepoService();
  ...
  it('returns early when refreshing auto sync without a repo', async () => {
    ...
    await expect(repoService.refreshAutoSyncSettings()).resolves.toBeUndefined();
  });
