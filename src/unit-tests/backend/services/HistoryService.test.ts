...
describe("HistoryService", () => {
    ...
    historyService = new HistoryService(
      {
        ...
        s3: mockS3HistoryProvider,
        local: mockLocalHistoryProvider,
      },
      mockConfigService,
    );
  ...
  describe("getVersion", () => {
    it("should get file content at specific commit", async () => {
      ...
      const content = await historyService.getVersion(
        "abc123",
        "notes/file.md",
      );

      expect(mockGitAdapter.show).toHaveBeenCalledWith(
        "abc123",
        "notes/file.md",
      );
...
+New line
 Line 3`;
      ...
      const diff = await historyService.getDiff(
        "abc123",
        "def456",
        "notes/file.md",
      );

      expect(mockGitAdapter.diff).toHaveBeenCalledWith(
        "abc123",
        "def456",
        "notes/file.md",
      );
...
+New line`;
  ...
  describe("s3 provider selection", () => {
    it("uses the s3 provider when repo settings are s3", async () => {
      ...
      expect(mockS3HistoryProvider.configure).toHaveBeenCalledWith(
        repoSettings,
      );
      expect(mockS3HistoryProvider.getForFile).toHaveBeenCalledWith(
        "notes/file.md",
      );
    });
  });
  ...
  describe("local provider selection", () => {
    it("uses the local provider when repo settings are local", async () => {
      ...
      expect(mockLocalHistoryProvider.configure).toHaveBeenCalledWith(
        repoSettings,
      );
      expect(mockLocalHistoryProvider.getForFile).toHaveBeenCalledWith(
        "notes/file.md",
      );
    });
  });
