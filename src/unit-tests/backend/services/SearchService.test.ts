...
describe('SearchService', () => {
  ...
  describe('search', () => {
    ...
    it('finds matches in content and file name', async () => {
      ...
      mockFsAdapter.readdir.mockImplementation(async (dir: string) => dirs[dir] || []);
      mockFsAdapter.stat.mockImplementation(async (fullPath: string) => ({
      ...
      expect(paths).toEqual(['notes/a.md', 'notes/c.markdown', 'notes/sub/inner.md']);
  ...
  describe('searchRepoWide', () => {
    ...
    it('supports regex fallback when regex is invalid', async () => {
      ...
      mockFsAdapter.readFile.mockImplementation(async (filePath: string) => files[filePath]);

      const results = await searchService.searchRepoWide('[', { useRegex: true });
