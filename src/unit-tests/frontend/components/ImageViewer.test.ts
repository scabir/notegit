...
const findToolbarButton = (renderer: TestRenderer.ReactTestRenderer, title: string) => {
...
describe('ImageViewer', () => {
  it('renders empty state when no file is selected', () => {
    ...
    expect(flattenText(renderer.toJSON())).toContain(IMAGE_VIEWER_TEXT.emptyState);
  });
  ...
  it('normalizes windows paths when building image URL', () => {
    ...
    const image = renderer.root.findByType('img');
    expect(image.props.src).toBe('file:///C:/repo/notes/assets/photo%20one.jpg');
  });
  ...
  it('shows tree controls and wires callbacks', () => {
    ...
    const showTreeButton = findToolbarButton(renderer, IMAGE_VIEWER_TEXT.showTreeTooltip);
    const backButton = findToolbarButton(renderer, IMAGE_VIEWER_TEXT.backTooltip);
    const forwardButton = findToolbarButton(renderer, IMAGE_VIEWER_TEXT.forwardTooltip);
