...
import { Tooltip, IconButton, MenuItem, ToggleButtonGroup } from '@mui/material';
import { MarkdownEditor } from '../../../frontend/components/MarkdownEditor';
import { MARKDOWN_EDITOR_TEXT, MARKDOWN_INSERT_DEFAULTS, MARKDOWN_INSERT_TOKENS } from '../../../frontend/components/MarkdownEditor/constants';
...
jest.mock('../../../frontend/components/MermaidDiagram', () => {
  const React = require('react');
  return {
    MermaidDiagram: (props: any) => React.createElement('div', { 'data-testid': 'mermaid', ...props }),
  };
});
...
const getToolbarButton = (renderer: TestRenderer.ReactTestRenderer, title: string) => {
...
describe('MarkdownEditor task list formatting', () => {
  beforeEach(() => {
    (global as any).window = {
      notegitApi: {
        config: {
          getAppSettings: jest.fn().mockResolvedValue({ ok: true, data: { editorPrefs: { lineNumbers: true } } }),
        },
        export: {
          note: jest.fn(),
        },
  ...
  it('creates a task per selected line', async () => {
    ...
    const button = getToolbarButton(renderer, MARKDOWN_EDITOR_TEXT.taskListLabel);
  ...
  it('inserts a table template', async () => {
    const { renderer } = await renderEditor('', 0, 0);
    const button = getToolbarButton(renderer, MARKDOWN_EDITOR_TEXT.tableTooltip);
  ...
  it('inserts a mermaid block from the extras menu', async () => {
    const { renderer } = await renderEditor('', 0, 0);
    const extrasButton = getToolbarButton(renderer, MARKDOWN_EDITOR_TEXT.extrasTooltip);
    ...
    const mermaidItem = renderer.root
      ...
      .find((item) => item.props.children === MARKDOWN_EDITOR_TEXT.mermaidLabel);
  ...
  it('shows the mermaid cheatsheet and allows closing it', async () => {
    ...
    const cheatsheetButton = getToolbarButton(renderer, MARKDOWN_EDITOR_TEXT.cheatsheetTooltip);
    ...
    const mermaidSheetItem = renderer.root
      .findAllByType(MenuItem)
      .find((item) => item.props.children === MARKDOWN_EDITOR_TEXT.mermaidCheatsheetLabel);
    ...
    const closeButtonTooltip = renderer.root
      .findAllByType(Tooltip)
      .find((item) => item.props.title === MARKDOWN_EDITOR_TEXT.closeCheatsheetLabel);
    ...
    const closeTooltips = renderer.root
      .findAllByType(Tooltip)
      .filter((item) => item.props.title === MARKDOWN_EDITOR_TEXT.closeCheatsheetLabel);
  ...
  it('exports and handles export errors', async () => {
    ...
    const { renderer } = await renderEditor('content', 0, 0);
    const exportButton = getToolbarButton(renderer, MARKDOWN_EDITOR_TEXT.exportTooltip);
  ...
  it('opens find bar and navigates/updates matches', async () => {
    ...
    const findBar = renderer.root.findByType(require('../../../frontend/components/FindReplaceBar').FindReplaceBar);
    await act(async () => {
  ...
  it('inserts footnotes, highlights, and raw markdown', async () => {
    ...
    const footnoteButton = getToolbarButton(renderer, MARKDOWN_EDITOR_TEXT.footnoteLabel);
    act(() => {
      footnoteButton.props.onClick();
    });

    const highlightButton = getToolbarButton(renderer, MARKDOWN_EDITOR_TEXT.highlightLabel);
    act(() => {
      highlightButton.props.onClick();
    });

    const definitionButton = getToolbarButton(renderer, MARKDOWN_EDITOR_TEXT.definitionListLabel);
    act(() => {
    ...
    const rawItem = renderer.root
      .findAllByType(MenuItem)
      .find((item) => item.props.children === MARKDOWN_EDITOR_TEXT.rawMarkdownLabel);
    if (!rawItem) {
  ...
  it('renders preview content with mermaid and image adjustments', async () => {
    const { renderer } = await renderEditor('![alt](image.png)\n\n```mermaid\ngraph TD\n```', 0, 0);

    const previewToggle = renderer.root
      .findAllByType(ToggleButtonGroup)[0];
    act(() => {
  ...
  it('opens markdown cheatsheet and closes it', async () => {
    ...
    const markdownSheetItem = renderer.root
      .findAllByType(MenuItem)
      .find((item) => item.props.children === MARKDOWN_EDITOR_TEXT.markdownCheatsheetLabel);
    ...
    expect(renderer.root.findAllByType(Tooltip).some((item) => item.props.title === MARKDOWN_EDITOR_TEXT.closeCheatsheetLabel)).toBe(
      false
    );
  });
  ...
  it('handles splitter drag in split view', async () => {
    ...
    const splitter = renderer.root.findAll(
      (node) => node.props.onMouseDown && node.props.sx?.cursor === 'col-resize'
    )[0];
    act(() => {
