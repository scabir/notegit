...
jest.mock('@uiw/react-codemirror', () => {
  ...
  return React.forwardRef((props: any, ref: any) => {
    ...
    return React.createElement('div', { 'data-testid': 'codemirror', ...props });
  });
});
...
describe('TextEditor', () => {
  ...
  it('enables save button when content changes', async () => {
    ...
    const saveButton = getToolbarButton(renderer!, TEXT_EDITOR_TEXT.saveTooltip);
    expect(saveButton.props.disabled).toBe(true);

    const editorNode = renderer!.root.findByProps({ 'data-testid': 'codemirror' });
    act(() => {
    ...
    const updatedSaveButton = getToolbarButton(renderer!, TEXT_EDITOR_TEXT.saveTooltip);
  ...
  it('alerts when export fails', async () => {
    ...
    const exportButton = getToolbarButton(renderer!, TEXT_EDITOR_TEXT.exportTooltip);
    await act(async () => {
  ...
  it('uses keymap handlers for save and insert line break', async () => {
    ...
    const keymaps = lastCodeMirrorProps.extensions.find((ext: any) => Array.isArray(ext));
    const modSave = keymaps.find((binding: any) => binding.key === 'Mod-s');
    const ctrlEnter = keymaps.find((binding: any) => binding.key === 'Ctrl-Enter');
