...
const createResultWithMatches = (filePath: string, fileName: string, matchCount: number): SearchResult => ({
  filePath,
  fileName,
...
describe('SearchDialog', () => {
  ...
  it('queries and selects the first result on Enter', async () => {
    ...
    const queryMock = jest.fn().mockResolvedValue({ ok: true, data: [createResult()] });
  ...
  it('shows an error when search fails', async () => {
    const queryMock = jest.fn().mockResolvedValue({ ok: false, error: { message: 'search failed' } });
    (global as any).window.notegitApi.search.query = queryMock;
  ...
  it('navigates results with arrow keys and selects via Enter', async () => {
    ...
    act(() => {
      textField.props.onKeyDown({ key: 'ArrowDown', preventDefault: jest.fn() });
    });
    await act(async () => {
      await Promise.resolve();
    });
    ...
    expect(chips.some((chip) => String(chip.props.label).includes('+1'))).toBe(true);
  });
  ...
  it('handles query errors and resets on close', async () => {
    ...
    expect(JSON.stringify(renderer!.toJSON())).toContain(SEARCH_DIALOG_TEXT.startTyping);
  });
