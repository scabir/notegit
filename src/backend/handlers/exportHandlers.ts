...
export function registerExportHandlers(ipcMain: IpcMain, exportService: ExportService): void {
  ipcMain.handle(
    'export:note',
    async (
      _event,
      ...
      try {
        const exportPath = await exportService.exportNote(fileName, content, defaultExtension);
        return {
          ok: true,
          data: exportPath,
        };
